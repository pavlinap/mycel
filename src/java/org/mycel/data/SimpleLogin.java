...
public class SimpleLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        long userID = UserManager.findUser(username,password);
        if (userID > 0){
            HttpSession session = request.getSession(true);
            session.setAttribute("currentSessionUsername",username);
            session.setAttribute("currentSessionUserID",userID);
            //User[] allUsers = UserManager.getUser();
            session.setAttribute("currentSessionUser", UserManager.getUserById(userID));
